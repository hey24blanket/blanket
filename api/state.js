...
const TOKEN = process.env.GITHUB_TOKEN || process.env.GITHUB_ACCESS_TOKEN || process.env.desk_GITHUB_TOKEN;
...
module.exports = async function handler(req, res) {
  if (!TOKEN) {
    return send(res, 503, {
      error: 'Vercel에 GitHub 토큰 공유 변수가 아직 연결되지 않았습니다. 로컬 자동저장은 정상 작동합니다.'
    });
  }
